...
/**
 * Utility function to transform the input into a url-friendly
 * "slug". For example, when given a file name such as
 * `My Blog Title.md` the slug becomes `my-blog-title`.
 * Ported from `https://github.com/simov/slugify` with
 * some added options.
 */
export const slugify = (str: string, opts: SlugifyOptions = {}) => {
  if (typeof str !== 'string') {
    throw new Error('slugify: string argument expected');
  }

  if (opts.removeFileExtension !== false) {
    const dotSplit = str.split('.');
    const lastPart = dotSplit.pop();
    if (lastPart && removeSlugExts[lastPart.toLowerCase()]) {
      str = dotSplit.join('.');
    }
  }

  const locale = slugifyLocales[opts.locale!] || {};
  ...
  str = str
    ...
    .reduce(
      (result, ch) =>
        result +
        (locale[ch] || slugifyCharMap[ch] || ch)
  ...
  if (opts.trimReplacement !== false) {
    if (str.length > 2 && str.startsWith(replacement)) {
      str = str.substring(1);
    }
    if (str.length > 2 && str.endsWith(replacement)) {
...
const slugifyCharMap = JSON.parse(
...
const slugifyLocales = JSON.parse(
  '{"de":{"Ä":"AE","ä":"ae","Ö":"OE","ö":"oe","Ü":"UE","ü":"ue"},"vi":{"Đ":"D","đ":"d"}}',
);

const removeSlugExts: { [key: string]: true } = {
  md: true,
  markdown: true,
  html: true,
  htm: true,
